...
describe('processRecord', () => {
    ...
    it('handles mockValidateEmail failing and updates the DB', async () => {
        mockValidateEmail.mockRejectedValue({msg: 'some error message'})
        const result = {
            email: 'breaking@email.com'
        }
        const failedDetails = {
            ...result,
            error: 'some error message'
        }
        await processRecord(result, 'testId3')
        expect(mockValidateEmail).toHaveBeenCalledWith('breaking@email.com')
        expect(updateUploadRecordById).toHaveBeenCalledWith('testId3', false, failedDetails)
    }
    )
})